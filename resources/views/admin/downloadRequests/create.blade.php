...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.downloadRequest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.download-requests.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="download">{{ trans('cruds.downloadRequest.fields.download') }}</label>
                <input class="form-control {{ $errors->has('download') ? 'is-invalid' : '' }}" type="text" name="download" id="download" value="{{ old('download', '') }}">
