...
<div class="amtex-prodf-page">
  ...
  <div class="amtex-prodf-header">
    <div class="amtex-prodf-header-left">
      ...
      <h2 class="amtex-prodf-title">{{ trans('global.create') }} {{ trans('cruds.product.title_singular') }}</h2>
      <p class="amtex-prodf-subtitle">Create a product with pricing, media, brochure and visibility controls.</p>
    </div>
  ...
  <div class="amtex-prodf-wrap">
    ...
    <div class="amtex-prodf-card">
      <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="amtex-prodf-form">
        @csrf
          ...
          <div class="amtex-prodf-grid">
            ...
            <div class="amtex-prodf-field">
              ...
              <div class="amtex-prodf-inlinehint">
                ...
                Slug is auto-generated from Name while typing. You can edit it manually anytime.
              </div>
            </div>
        ...
        <div class="amtex-prodf-section">
          ...
          <div class="amtex-prodf-grid amtex-prodf-grid-1">
            <div class="amtex-prodf-field">
              ...
              <textarea class="form-control ckeditor amtex-prodf-control {{ $errors->has('short_desc') ? 'is-invalid' : '' }}" name="short_desc" id="short_desc">{!! old('short_desc') !!}</textarea>
            ...
            <div class="amtex-prodf-field">
              ...
              <textarea class="form-control ckeditor amtex-prodf-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description') !!}</textarea>
        ...
        <div class="amtex-prodf-section">
          ...
          <div class="amtex-prodf-subcard">
            ...
            <div class="amtex-prodf-grid">
              ...
              <div class="amtex-prodf-field">
                ...
                <input class="form-control amtex-prodf-control {{ $errors->has('pdp_available_sizes') ? 'is-invalid' : '' }}" id="pdp_available_sizes" name="pdp_available_sizes" value="{{ old('pdp_available_sizes','') }}" placeholder="e.g. 1L, 2L, 3L, 4L, 6L, 9L">
              ...
              <div class="amtex-prodf-field">
                ...
                <input class="form-control amtex-prodf-control {{ $errors->has('pdp_available_variants') ? 'is-invalid' : '' }}" id="pdp_available_variants" name="pdp_available_variants" value="{{ old('pdp_available_variants','') }}" placeholder="e.g. Stainless Steel Polish, Regular Fire Red">
            ...
            {{-- Hidden arrays --}}
            <input type="hidden" name="pdp_recommended_for_left" id="amtexRecoLeftJson" value="{{ old('pdp_recommended_for_left','') }}">
            <input type="hidden" name="pdp_recommended_for_right" id="amtexRecoRightJson" value="{{ old('pdp_recommended_for_right','') }}">
          ...
          <div class="amtex-prodf-subcard">
            ...
            <div class="amtex-prodf-techShell" id="amtexTechShell">
              ...
              {{-- ✅ Mobile card view (auto generated) --}}
              <div id="amtexTechCards" class="amtex-prodf-techCards d-none"></div>
            </div>

            <input type="hidden" name="pdp_tech_table" id="amtexTechJson" value="{{ old('pdp_tech_table','') }}">
        ...
        <div class="amtex-prodf-section">
          ...
          <div class="amtex-prodf-grid">
            ...
            <div class="amtex-prodf-field">
              ...
              <select class="form-control amtex-prodf-control {{ $errors->has('badge') ? 'is-invalid' : '' }}" name="badge" id="badge" required>
                ...
                @foreach(App\Models\Product::BADGE_SELECT as $key => $label)
                  <option value="{{ $key }}" {{ old('badge', 'New') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
              </select>
              @if($errors->has('badge'))
            ...
            <div class="amtex-prodf-field">
              ...
              <input class="form-control amtex-prodf-control {{ $errors->has('rating_avg') ? 'is-invalid' : '' }}" type="text" name="rating_avg" id="rating_avg" value="{{ old('rating_avg', '') }}" placeholder="e.g. 4.8">
            ...
            <div class="amtex-prodf-field">
              ...
              <input class="form-control amtex-prodf-control {{ $errors->has('rating_count') ? 'is-invalid' : '' }}" type="text" name="rating_count" id="rating_count" value="{{ old('rating_count', '') }}" placeholder="e.g. 120">
            ...
            <div class="amtex-prodf-field">
              ...
              <input class="form-control amtex-prodf-control {{ $errors->has('sku') ? 'is-invalid' : '' }}" type="text" name="sku" id="sku" value="{{ old('sku', '') }}" placeholder="e.g. AMTEX-ABC-01">
              @if($errors->has('sku'))
            ...
            <div class="amtex-prodf-field">
              ...
              <select class="form-control amtex-prodf-control {{ $errors->has('is_featured') ? 'is-invalid' : '' }}" name="is_featured" id="is_featured" required>
                ...
                @foreach(App\Models\Product::IS_FEATURED_SELECT as $key => $label)
                  <option value="{{ $key }}" {{ old('is_featured', 'no') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
              </select>
          ...
          <div class="amtex-prodf-grid">
              ...
              <div class="amtex-prodf-dropwrap {{ $errors->has('main_image') ? 'amtex-prodf-dropwrap-error' : '' }}">
                ...
                <div class="amtex-prodf-dropnote">
                  <div class="amtex-prodf-dropnote-title">
                    <i class="fas fa-cloud-upload-alt"></i> Upload Main Image
                  </div>
            ...
            <div class="amtex-prodf-field amtex-prodf-span2">
              ...
              <div class="amtex-prodf-dropwrap {{ $errors->has('brochure_pdf') ? 'amtex-prodf-dropwrap-error' : '' }}">
                ...
                <div class="amtex-prodf-dropnote">
                  <div class="amtex-prodf-dropnote-title">
                    <i class="fas fa-file-pdf"></i> Upload Brochure PDF
                  </div>
                  <div class="amtex-prodf-dropnote-sub">
...
<script>
  // -------------------------------
  // CKEditor Upload Adapter (kept same, but safe for create)
  // -------------------------------
          ...
          upload: function() {
            return loader.file.then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                data.append('crud_id', '0'); // ✅ create page
                xhr.send(data);
              });
            })
          }
...
<script>
  ...
  Dropzone.options.brochurePdfDropzone = {
    ...
    success: function (file, response) {
      ...
      $('#amtexProdPrevPdf').html(
        '<div class="amtex-prodf-preview-filetext"><i class="fas fa-file-pdf"></i> ' + fileName + ' uploaded</div>'
      );
    },
...
<script>
  ...
  $(document).ready(function () {
    if ($('#slug').val().trim() !== '') {
      amtexProdSlugManuallyEdited = true; // respect old value after validation failure
    }
    amtexProdUpdatePreview();
  });
</script>
...
<script>
  /**
   * PDP Builder: Repeaters + Technical table
   * - tags: pdp_tags[]
   * - pointers: pdp_pointers[]
   * - key highlights: pdp_key_highlights[index][icon/title/text]
   * - works on: pdp_works_on[index][icon/title/text]
   * - recommended for: textarea -> JSON hidden inputs
   * - tech table: builder -> JSON hidden
   *
   * ✅ Improvements in point 6:
   * - Horizontal scroll shell (mobile safe)
   * - Compact view => converts to card view (best on phone)
   * - Sticky parameter column in full table mode
   * - Search filter (parameter/value)
   * - Better “Add column/row” UX preserved
   */
  function amtexEscapeHtml(s){ return (s||'').replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])); }

  function amtexAddRow(wrapper, html){
    wrapper.insertAdjacentHTML('beforeend', html);
  }

  function amtexBindRemove(){
    document.querySelectorAll('[data-amtex-remove]').forEach(btn=>{
      btn.onclick = () => btn.closest('.amtex-prodf-row').remove();
    });
  }
  ...
  // ---------- RECOMMENDED FOR (convert textarea lines -> JSON string) ----------
  ...
  // ---------- TECH TABLE BUILDER (Responsive + Friendly) ----------
  ...
  function renderTechTable(){
    ...
    if (!filteredRows.length) {
      html += `
        <div class="amtex-prodf-techEmpty">
          No matching rows for "<b>${amtexEscapeHtml(techFilter)}</b>".
        </div>
      `;
    }
    ...
    techWrap.querySelectorAll('[data-tech-val]').forEach(inp=>{
      inp.oninput = () => {
        ...
        // no full re-render (keeps typing smooth)
      };
    });
  ...
  function renderTechCards(){
    ...
    // bind value edits (reuse same state keys)
  ...
  // --------- On submit: finalize JSON fields ----------
  ...
  function amtexTryParseJson(val){
  ...
  $(document).ready(function () {
    ...
    // Tags restore
    const oldTags = @json(old('pdp_tags', []));
    if (Array.isArray(oldTags) && oldTags.length) {
      oldTags.forEach(t=>{
        $('#amtexTagAdd').trigger('click');
        const last = $('#amtexTagsWrap input[name="pdp_tags[]"]').last();
        last.val(t);
      });
    } else {
      // defaults
    ...
    // Pointers restore
    const oldPointers = @json(old('pdp_pointers', []));
    if (Array.isArray(oldPointers) && oldPointers.length) {
      oldPointers.forEach(p=>{
        $('#amtexPointerAdd').trigger('click');
        const last = $('#amtexPointersWrap input[name="pdp_pointers[]"]').last();
        last.val(p);
      });
    } else {
      // default 3
    ...
    // Key highlights restore
    const oldKey = @json(old('pdp_key_highlights', []));
    if (Array.isArray(oldKey) && oldKey.length) {
      oldKey.forEach((h)=>{
    ...
    // Works on restore
    const oldWorks = @json(old('pdp_works_on', []));
    if (Array.isArray(oldWorks) && oldWorks.length) {
      oldWorks.forEach((w)=>{
    ...
    // Recommended-for restore (if old json exists, also paint in textarea)
    const oldRecoLeftJson = $('#amtexRecoLeftJson').val();
    const oldRecoRightJson = $('#amtexRecoRightJson').val();

    const leftArr = amtexTryParseJson(oldRecoLeftJson);
    const rightArr = amtexTryParseJson(oldRecoRightJson);

    if (Array.isArray(leftArr) && leftArr.length && !$('#amtexRecoLeftRaw').val()) {
      $('#amtexRecoLeftRaw').val(leftArr.join("\n"));
    }
    if (Array.isArray(rightArr) && rightArr.length && !$('#amtexRecoRightRaw').val()) {
      $('#amtexRecoRightRaw').val(rightArr.join("\n"));
    }

    // Tech table restore
    const oldTechJson = $('#amtexTechJson').val();
    const techObj = amtexTryParseJson(oldTechJson);

    if (techObj && typeof techObj === 'object' && Array.isArray(techObj.columns) && Array.isArray(techObj.rows)) {
      techState = techObj;
    } else {
      // default starter like sample
    ...
    // default: full table on desktop, compact on very small screens
...
{{-- ✅ Minimal CSS (inline) for responsive tech editor --}}
<style>
  .amtex-prodf-techbar{
  ...
  .amtex-prodf-techColHead{
    display:flex; gap:8px; align-items:center;
  }
  .amtex-prodf-chipDel{
    border:0; background:transparent; opacity:.7; padding:0 6px; cursor:pointer;
  }
  .amtex-prodf-techParamWrap{
    display:flex; gap:8px; align-items:center;
  }
  .amtex-prodf-rowDelSmall{
    width:34px; height:34px; display:inline-flex; align-items:center; justify-content:center;
  }
  .amtex-prodf-techEmpty{
    padding:14px; opacity:.8;
  }

  /* Compact cards (mobile friendly) */
  .amtex-prodf-techCards{
  ...
  .amtex-prodf-techCardBody{
    display:grid; gap:10px;
    grid-template-columns: 1fr;
  }
  .amtex-prodf-techCardItem{
    display:grid; gap:6px;
  }
  .amtex-prodf-techCardLabel{
    font-size:.85rem; opacity:.75; font-weight:700;
  }

  /* When compact mode enabled, keep scroll border consistent */
  .amtex-tech-compact .amtex-prodf-techScroll{
