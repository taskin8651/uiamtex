...
<div class="amtex-faq-form">
    ...
    <div class="af-header">
        <div>
            <h3 class="af-title">{{ trans('global.edit') }} {{ trans('cruds.faq.title_singular') }}</h3>
            <p class="af-subtitle">Update FAQ content, category and publishing settings.</p>
        </div>
        ...
        <div style="display:flex; gap:.5rem; flex-wrap:wrap;">
            ...
            @can('faq_show')
                <a href="{{ route('admin.faqs.show', $faq->id) }}" class="btn btn-outline-primary">
                    View
                </a>
            @endcan
        </div>
            ...
            <div class="col-lg-8 mb-3">
                <div class="card af-card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group af-field">
                            ...
                            >
                                @foreach($select_categories as $id => $entry)
                                    <option value="{{ $id }}"
                                        {{ ((old('select_category_id') ? old('select_category_id') : ($faq->select_category->id ?? '')) == $id) ? 'selected' : '' }}>
                                        {{ $entry }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="af-note">
                            <strong>Tip</strong>
                            Keep the question concise and make the answer structured (bullets/steps) for better readability.
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="form-group af-field">
                            ...
                            <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active">
                                ...
                                @foreach(App\Models\Faq::IS_ACTIVE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('is_active', $faq->is_active) === (string) $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
