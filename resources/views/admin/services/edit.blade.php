...
<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between">
        <span>{{ trans('global.edit') }} Service</span>
        ...
        <form method="POST" action="{{ route('admin.services.update', $service->id) }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}"
                           type="text" name="category" value="{{ old('category', $service->category) }}">
                ...
                <div class="col-md-4">
                    ...
                    <input class="form-control {{ $errors->has('cta_label') ? 'is-invalid' : '' }}"
                           type="text" name="cta_label" value="{{ old('cta_label', $service->cta_label) }}">
                ...
                <div class="col-md-4">
                    ...
                    <input class="form-control {{ $errors->has('sort_order') ? 'is-invalid' : '' }}"
                           type="number" name="sort_order" value="{{ old('sort_order', $service->sort_order) }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" required>
                        @foreach($isActiveOptions as $key => $label)
                            <option value="{{ $key }}" {{ old('is_active', $service->is_active)===(string)$key?'selected':'' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div id="featuresWrap" class="mt-2">
                @php
                    ...
                    $featuresToRender = is_array($oldFeatures) ? $oldFeatures : $service->features->map(function($f){
                        return ['feature_text' => $f->feature_text, 'sort_order' => $f->sort_order];
                    })->toArray();
                @endphp

                @if(count($featuresToRender) === 0)
                    @php $featuresToRender = [['feature_text' => '', 'sort_order' => 1]]; @endphp
                @endif

                @foreach($featuresToRender as $i => $row)
                    <div class="feature-row border rounded p-2 mb-2">
                        <div class="row g-2 align-items-center">
                            <div class="col-md-9">
                                <input type="text" data-role="feature_text"
                                       name="features[{{ $i }}][feature_text]" class="form-control"
                                       value="{{ $row['feature_text'] ?? '' }}"
                                       placeholder="e.g. Annual maintenance & refilling support">
                            </div>
                            <div class="col-md-2">
                                <input type="number" data-role="sort_order"
                                       name="features[{{ $i }}][sort_order]" class="form-control"
