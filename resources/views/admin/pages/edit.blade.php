...
<div class="card page-edit-card">
    <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
            <strong>{{ trans('global.edit') }} {{ trans('cruds.page.title_singular') }}</strong>
            <div class="text-muted small mt-1">
                Update page content, SEO metadata, status, and placement.
            </div>
        </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.pages.update', [$page->id]) }}" enctype="multipart/form-data" id="pageEditForm">
            @method('PUT')
            @csrf
                            ...
                            <div class="form-group mb-0">
                                ...
                                >
                                    ...
                                    @foreach(App\Models\Page::IS_ACTIVE_SELECT as $key => $label)
                                        <option value="{{ $key }}" {{ old('is_active', $page->is_active) === (string) $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mb-3">
                                <div class="custom-control custom-switch">
                                    <input
                                        ...
                                        name="show_in_navbar"
                                        value="1"
                                        {{ old('show_in_navbar', (int) $page->show_in_navbar) ? 'checked' : '' }}
                                    >
                            ...
                            <div class="form-group mb-3">
                                <div class="custom-control custom-switch">
                                    <input
                                        ...
                                        name="show_in_footer"
                                        value="1"
                                        {{ old('show_in_footer', (int) $page->show_in_footer) ? 'checked' : '' }}
                                    >
                            ...
                            <div class="form-group mb-0">
                                <div class="custom-control custom-switch">
                                    <input
                                        ...
                                        value="1"
                                        {{ old('show_in_footer_bottom', (int) $page->show_in_footer_bottom) ? 'checked' : '' }}
                                    >
                    ...
                    <div class="card inner-card">
                        <div class="card-body">
                            <div class="small text-muted">
                                Last updated:
                                <strong>{{ optional($page->updated_at)->format('d M Y, h:i A') }}</strong>
                            </div>
...
@parent
<style>
    .page-edit-card .inner-card { border: 1px solid rgba(0,0,0,.08); }
    .page-actions{
        position: sticky;
        bottom: 0;
        background: #fff;
...
$(document).ready(function () {
    ...
    // ---------- Optional: Slug auto-generate from title ONLY if user hasn't edited slug ----------
    let initialSlug = $('#slug').val().trim();
    ...
    $('#slug').on('input', function () {
        slugTouched = $(this).val().trim() !== initialSlug;
    });
                    ...
                    return loader.file.then(function (file) {
                        return new Promise(function(resolve, reject) {
                            ...
                            xhr.addEventListener('load', function() {
                                ...
                                $('#pageEditForm').append('<input type="hidden" name="ck-media[]" value="' + response.id + '">');
