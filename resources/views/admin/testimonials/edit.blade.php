...
<div class="amtex-page">
    <div class="amtex-header">
        <div>
            <h2 class="amtex-title">Edit Testimonial</h2>
            <p class="amtex-subtitle">Update testimonial details.</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.testimonials.update', $testimonial->id) }}" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="company_designation" value="{{ old('company_designation', $testimonial->company_designation) }}"
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="is_active" class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" required>
                            @foreach(\App\Models\Testimonial::IS_ACTIVE_SELECT as $key => $label)
                                <option value="{{ $key }}" {{ old('is_active', $testimonial->is_active) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
