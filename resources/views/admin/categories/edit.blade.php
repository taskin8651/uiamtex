...
<div class="amtex-catf-page">
  ...
  <div class="amtex-catf-header">
    <div class="amtex-catf-header-left">
      ...
      <h2 class="amtex-catf-title">{{ trans('global.edit') }} {{ trans('cruds.category.title_singular') }}</h2>
      <p class="amtex-catf-subtitle">Update category details, image, sort priority and visibility.</p>
    </div>
  ...
  <div class="amtex-catf-wrap">
    ...
    <div class="amtex-catf-card">
      <form method="POST" action="{{ route('admin.categories.update', [$category->id]) }}" enctype="multipart/form-data" class="amtex-catf-form">
        @method('PUT')
        @csrf
          ...
          <div class="amtex-catf-grid">
            ...
            <div class="amtex-catf-field">
              ...
              <div class="amtex-catf-inlinehint">
                ...
                On edit, slug won’t change automatically when you type Name. Use refresh to regenerate.
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="amtex-catf-section-head">
            <div class="amtex-catf-section-title">
              ...
              <div>
                ...
                <div class="amtex-catf-hint">Replace image, set ordering, and status.</div>
              </div>
            </div>
              ...
              <div class="amtex-catf-dropwrap {{ $errors->has('image') ? 'amtex-catf-dropwrap-error' : '' }}">
                ...
                <div class="amtex-catf-dropnote">
                  <div class="amtex-catf-dropnote-title">
                    <i class="fas fa-cloud-upload-alt"></i> Upload / Replace Category Image
                  </div>
              ...
              <div class="amtex-catf-input">
                ...
                >
                  ...
                  @foreach(App\Models\Category::IS_ACTIVE_SELECT as $key => $label)
                    <option value="{{ $key }}" {{ old('is_active', (string) $category->is_active) === (string) $key ? 'selected' : '' }}>
                      {{ $label }}
                    </option>
                  @endforeach
                </select>
...
<script>
  ...
  Dropzone.options.imageDropzone = {
    ...
    init: function () {
      @if(isset($category) && $category->image)
        ...
        // robust preview url selection
        var previewUrl = file.preview ?? file.preview_url ?? file.url ?? (file.original_url ?? null);

        if (previewUrl) {
          this.options.thumbnail.call(this, file, previewUrl);
        }
        ...
        if (previewUrl) {
          $('#amtexCatPrevImg').html('<img src="' + previewUrl + '" alt="Preview" />');
        }
      @endif
    },
  ...
  // -------------------------------
  // Slug behavior on edit:
  // - DO NOT auto-change while typing name (safer)
  // - Provide "Regenerate" button
  // -------------------------------
  let amtexSlugManuallyEdited = true; // edit mode default true
  ...
  $(document).ready(function () {
    // keep existing slug; if validation failed and slug empty, allow regenerate behavior
    amtexSlugManuallyEdited = ($('#slug').val().trim() !== '');
  ...
  $('#name').on('input', function () {
    // in edit, do NOT override slug automatically
    amtexCatUpdatePreview();
  });
  ...
  $('#slug').on('blur', function () {
    if ($(this).val().trim() === '') {
      // if cleared, we can regenerate to avoid empty slug submission
      amtexSlugManuallyEdited = false;
      $('#slug').val(amtexSlugify($('#name').val()));
      amtexCatUpdatePreview();
    }
