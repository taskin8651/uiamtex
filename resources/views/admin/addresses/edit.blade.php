    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.addresses.update", [$address->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('label') ? 'is-invalid' : '' }}" type="text" name="label" id="label" value="{{ old('label', $address->label) }}" required>
                @if($errors->has('label'))
                    <div class="invalid-feedback">
                        {{ $errors->first('label') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('line_1') ? 'is-invalid' : '' }}" type="text" name="line_1" id="line_1" value="{{ old('line_1', $address->line_1) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_default') ? 'is-invalid' : '' }}" name="is_default" id="is_default">
                    ...
                    @foreach(App\Models\Address::IS_DEFAULT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_default', $address->is_default) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
